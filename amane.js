...
module.exports = amane = async (amane, m, chatUpdate, store) => {
try {
    if (!m || !m.message) return;

    m.chat = m.key.remoteJid || '';
    m.isGroup = m.chat.endsWith('@g.us');
    m.sender = m.key.fromMe ? (amane.user.id.split(':')[0]+'@s.whatsapp.net' || amane.user.id) : (m.key.participant || m.key.remoteJid || '');
    ...
    const msgHelper = require('./lib/src/message')(amane, m, chatUpdate, store);
    m = msgHelper.m;
    ...
    if (rawContext && rawContext.quotedMessage) {
        ...
        m.quoted = {
            key: {
                remoteJid: m.chat,
                fromMe: rawContext.participant === amane.user.id.split(':')[0] + '@s.whatsapp.net',
    ...
    const botNumber = await amane.decodeJid(amane.user.id)
    ...
    store.groupMetadata = store.groupMetadata || {};
        ...
        if (m.isGroup) {
            ...
            if (Object.keys(store.groupMetadata).length === 0 || invalidMembers.length >= 1) {
                store.groupMetadata = await amane.groupFetchAllParticipating();
            }
        }

        const groupMetadata = m.isGroup
            ? store.groupMetadata[m.chat]
            || (store.groupMetadata[m.chat] = await amane.groupMetadata(m.chat).catch(e => {}))
            : '';
...
await antilinkDetector(amane, m, { budy, type, isAdmins, Ahmad, isBotAdmins, sender, senderNumber });
...
➤ 👤 Nama : *${pushname}*
➤ 👑 Jabatan : *${Ahmad ? 'Owner 👨‍💻' : 'Free User'}*
➤ 👥 Total Pengguna : *${Object.keys(global.db.users).length}*
➤ ⭐ Rating : *${rata2}* dari *${totalRating}* pengguna
➤ ⚒️ Total Fitur : *${amaneFitur()} fitur*

🌿━━━〔 🧷 *Media Sosial* 〕━━━🌿

➤ 🪀 WhatsApp : *wa.me/${global.ownernomer}*
➤ 📸 Instagram : *www.instagram.com/${global.ig}*

🌿━━━〔 🤖 *Info Bot* 〕━━━🌿

➤ 🤖 Nama Bot : *${global.botname}*
➤ ⏱️ Aktif Selama : *${runtime(process.uptime())}*
...
🚀 *Powered By ${global.botname}*`)
        ...
        await listbut2(amane, m, teks, bet)
        ...
        if (global.music && typeof global.music === 'string' && global.music.trim() !== '') {
            ...
            if (isAudioValid) {
                try {
                    await amane.sendMessage(m.chat, { audio: audioSource, mimetype: 'audio/mp4', ptt: true }, { quoted: m });
                } catch (e) {
                }
            }
        }
...
// ====== OWNER FEATURE ======
    ...
    case 'addowner': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            ...
            else return m.reply('reply pesannya, atau ketik nomornya!\nContoh: *.addowner 628xx*')
    ...
    case 'delowner': {
            ...
            if (!extraOwner.includes(num)) return m.reply('❌ Wahaha, Kamu dilarang menghapus nomor bawaan!')
    ...
    case 'public': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (global.db.settings.public) return m.reply('Bot sudah dalam mode Public!')
    ...
    case 'self': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (!global.db.settings.public) return m.reply('Bot sudah dalam mode Self!')
    ...
    case 'onlygc': case 'onlygroup': case 'onlygb': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (args[0] === 'on') {
                if (global.db.settings.onlygc) return m.reply('Bot sudah dalam mode Only Group!')
                global.db.settings.onlygc = true
                global.db.settings.onlypc = false
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
                replysuccess('Berhasil mengubah mode ke *Only Group*')
            } else if (args[0] === 'off') {
                if (!global.db.settings.onlygc) return m.reply('Mode Only Group memang sudah mati!')
    ...
    case 'onlypc': case 'onlyprivate': case 'onlypm': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (args[0] === 'on') {
                if (global.db.settings.onlypc) return m.reply('Bot sudah dalam mode Only Private Chat!')
                global.db.settings.onlypc = true
                global.db.settings.onlygc = false
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
                replysuccess('Berhasil mengubah mode ke *Only Private Chat*')
            } else if (args[0] === 'off') {
                if (!global.db.settings.onlypc) return m.reply('Mode Only Private Chat memang sudah mati!')
                global.db.settings.onlypc = false
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
    ...
    case 'towl': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (args[0] === 'on') {
                if (global.db.settings.whitelistMode) return m.reply('Mode Whitelist sudah aktif!')
                global.db.settings.whitelistMode = true
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
                replysuccess(global.mess.on)
            } else if (args[0] === 'off') {
                if (!global.db.settings.whitelistMode) return m.reply('Mode Whitelist memang sudah mati!')
                global.db.settings.whitelistMode = false
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
                replysuccess(global.mess.off)
            } else {
                m.reply(`${global.mess.query.text}\n\nPilih on atau off!\nContoh: *.towl on*`)
            }
        }
        break
    case 'addwl': {
            ...
            if (!target) return m.reply(`${global.mess.query.text}\n\nGunakan perintah ini di dalam grup, atau ketik ID Grupnya!\nContoh: *.addwl*`)
            
            if (global.db.settings.whitelist.includes(target)) return m.reply("⚠️ Grup ini sudah ada di *whitelist*.")
    ...
    case 'delwl': {
            ...
            if (!m.isGroup && !args[0]) {
                if (wl.length === 0) return m.reply('Daftar Whitelist saat ini kosong!')
                ...
                return await amane.relayMessage(msg.key.remoteJid, msg.message, {
            ...
            if (index === -1) return m.reply("⚠️ Grup ini tidak ada di *whitelist*.")
    ...
    case 'resetwl': {
            if (!Ahmad) return m.reply(global.mess.only.owner)
            if (global.db.settings.whitelist.length === 0) return m.reply('Daftar Whitelist sudah kosong dari awal!')
    ...
    await amane.groupAcceptInvite(result)
        .then((res) => {
    ...
    case 'addsewa': {
        ...
        try {
            const g = await amane.groupGetInviteInfo(inviteCode)
            const groupId = g.id
            ...
            try {
                await amane.groupAcceptInvite(inviteCode)
            } catch (err) {
                let errorStr = String(err)
    ...
    case 'delsewa': {
        ...
        if (targetId) {
            if (!global.db.sewa[targetId]) return m.reply("❌ Grup tidak ditemukan di database sewa!")

            try {
                await amane.groupLeave(targetId)
            } catch (e) {}
        ...
        if (sewaKeys.length === 0) return m.reply("❌ Tidak ada sewa aktif saat ini.")
        ...
        await amane.relayMessage(msg.key.remoteJid, msg.message, {
            messageId: msg.key.id
        });
    }
        break
    case 'listsewa': {
        if (!Ahmad) return m.reply(global.mess.only.owner)
        if (!global.db.sewa || Object.keys(global.db.sewa).length === 0) return m.reply("📭 Belum ada data sewa.")
    ...
    case 'creategroup': {
        ...
        replywait(global.mess.wait)
        try {
            let group = await amane.groupCreate(text, [])
            let code = await amane.groupInviteCode(group.id)
...
// ====== GROUP FEATURE ======
    ...
    case 'setprefixgc': {
            if (!m.isGroup) return m.reply(global.mess.only.group)
            if (!isGroupAdmins && !Ahmad) return m.reply(global.mess.only.admin)
            if (!q) return m.reply(`Masukkan prefix baru untuk grup ini!\nGunakan | untuk memisahkan prefix, ketik *noprefix* untuk tanpa prefix, atau ketik *reset* untuk kembali ke prefix global.\n\nContoh: *${prefix}setprefixgc !|#|noprefix*`)
    ...
    case 'ceksewa': {
        ...
        if (!global.db.sewa || !global.db.sewa[m.chat]) {
            return m.reply("❌ Grup ini belum menyewa bot.")
        }
    ...
    case 'jadwalsholat': {
        ...
        if (args[0] === 'on') {
            ...
            let text = q;
            if (!text) return m.reply(`❌ Masukkan nama Kota atau Kabupaten!\n\nContoh: \n*${prefix}jadwalsholat Metro*\n*${prefix}jadwalsholat on/off*`);
    ...
    case 'promote': {
        if (!m.isGroup) return m.reply(global.mess.only.group)
        if (!isGroupAdmins && !Ahmad) return m.reply(global.mess.only.admin)
        if (!isBotAdmins) return m.reply('❌ Gagal! Bot harus menjadi admin terlebih dahulu.')
        
        let users = m.quoted ? [m.quoted.sender] : text ? [text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'] : []
        if (!users[0]) return m.reply(`Balas pesan orangnya atau ketik nomornya!\nContoh: *${prefix}promote 6281234567890*`)
        
        await amane.groupParticipantsUpdate(m.chat, users, 'promote')
    ...
    case 'demote': {
        ...
        await amane.groupParticipantsUpdate(m.chat, users, 'demote')
    ...
    case 'kick': {
        ...
        await amane.groupParticipantsUpdate(m.chat, users, 'remove')
    ...
    case 'setnamegc': {
        ...
        if (!isGroupAdmins && !Ahmad) return m.reply(global.mess.only.admin)
        if (!isBotAdmins) return m.reply('❌ Gagal! Bot harus menjadi admin terlebih dahulu.')
        if (!text) return m.reply(`Masukkan nama grup yang baru!\nContoh: *${prefix}setnamegc amane Community*`)
        
        await amane.groupUpdateSubject(m.chat, text)
    ...
    case 'setdescgc': {
        ...
        if (!isGroupAdmins && !Ahmad) return m.reply(global.mess.only.admin)
        if (!isBotAdmins) return m.reply('❌ Gagal! Bot harus menjadi admin terlebih dahulu.')
        if (!text) return m.reply(`Masukkan deskripsi grup yang baru!\nContoh: *${prefix}setdescgc Patuhi Aturan Grup!*`)
        
        await amane.groupUpdateDescription(m.chat, text)
    ...
    case 'setppgc': {
        ...
        if (!isImageMsg) return m.reply(`Kirim gambar dengan caption *${prefix}setppgc* atau balas gambarnya!`)
        
        try {
            reply(global.mess.wait)
            let media = await amane.downloadMediaMessage(m.quoted ? m.quoted : m)
            await amane.updateProfilePicture(m.chat, media)
    ...
    case 'groupinfo': {
        ...
        if (args[0] === 'on') {
            if (currentStatus === true) return m.reply(`Fitur *${command}* sudah aktif!`)
            gc[command] = true
            fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
            replysuccess(`✅ Fitur *${command}* berhasil diaktifkan!`)
        } else if (args[0] === 'off') {
            if (currentStatus === false) return m.reply(`Fitur *${command}* memang sudah mati!`)
            gc[command] = false
            fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
            replysuccess(`❌ Fitur *${command}* berhasil dimatikan!`)
        } else {
            m.reply(`Status saat ini: *${currentStatus ? 'ON' : 'OFF'}*\n\nKetik *${prefix + command} on/off* untuk mengubah.`)
        }
    }
        break
    case 'mute':
    case 'onlyadmin': {
        ...
        if (args[0] === 'on') {
            if (gc.mute) return m.reply(`Bot sudah di-mute untuk member di grup ini!`)
            gc.mute = true
            fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
            replysuccess(global.mess.on)
        } else if (args[0] === 'off') {
            if (!gc.mute) return m.reply(`Bot memang tidak sedang di-mute!`)
            gc.mute = false
            fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2))
            replysuccess(global.mess.off)
        } else {
            m.reply(`Status Mute: *${gc.mute ? 'ON' : 'OFF'}*\n\nKetik \n*${prefix + command} on* untuk membisukan bot dari member.\n*${prefix + command} off* untuk mematikan mute.`)
        }
    }
        break
    case 'antilinkall':
    ...
    case 'antitoxic':
        ...
        if (args[0] === 'on') {
            if (gc.antilink[typeLink]) return m.reply(`⚠️ Anti *${typeLink.toUpperCase()}* sudah aktif sebelumnya!`);
            gc.antilink[typeLink] = true;
            replysuccess(`✅ Anti *${typeLink.toUpperCase()}* berhasil diaktifkan!`);
        } else if (args[0] === 'off') {
            if (!gc.antilink[typeLink]) return m.reply(`⚠️ Anti *${typeLink.toUpperCase()}* memang sudah mati!`);
            gc.antilink[typeLink] = false;
            replysuccess(`❌ Anti *${typeLink.toUpperCase()}* berhasil dimatikan!`);
        } else {
            m.reply(`Status Anti ${typeLink.toUpperCase()}: *${gc.antilink[typeLink] ? 'ON' : 'OFF'}*\nKetik: *${prefix + command} on/off*`);
        }
    ...
    case 'addbadword': {
        ...
        if (bw.includes(word)) return m.reply(`⚠️ Kata *${word}* sudah ada di dalam database toxic!`);
    ...
    case 'delbadword': {
        ...
        let word = text.toLowerCase().trim();
        if (!bw.includes(word)) return m.reply(`⚠️ Kata *${word}* tidak ditemukan di dalam database toxic!`);
    ...
    case 'listbadword': {
        ...
        if (!fs.existsSync('./database/badword.json')) {
            return m.reply('📂 Daftar kata toxic masih kosong.');
        }
        ...
        await amane.sendMessage(m.chat, { text: textList }, { quoted: m });
    }
        break;
    case 'setantilink': {
        if (!m.isGroup) return m.reply(global.mess.only.group);
...
// ====== DOWNLOADER FEATURE ======
    ...
    case 'tiktok':
    case 'tt': {
        if (!text) return m.reply(`📌 Contoh: ${prefix + command} https://vt.tiktok.com/...`)
        ...
        try {
            ...
            if (images.length > 0) {
                let cards = await Promise.all(images.map(async (url, i) => ({
                    header: proto.Message.InteractiveMessage.Header.create({
                        ...(await prepareWAMessageMedia({ image: { url } }, { upload: amane.waUploadToServer })),
                        title: '',
                ...
                await amane.relayMessage(m.chat, msg.message, { messageId: msg.key.id })
            } else {
                ...
                await amane.sendMessage(m.chat, {
            ...
            if (audioUrl) {
                try {
                    ...
                    await amane.sendMessage(m.chat, { audio: Buffer.from(resAudio.data), mimetype: 'audio/mp4' }, { quoted: m });
                    audioSent = true;
                } catch (e) {
                }
            }
            ...
            if (!audioSent && images.length === 0 && videoObj && videoObj.url) {
                try {
                    ...
                    await amane.sendMessage(m.chat, { audio: fs.readFileSync(tmpAud), mimetype: 'audio/mp4' }, { quoted: m });
    ...
    case 'ttaudio': {
        ...
        try {
            ...
            if (audioUrl) {
                try {
                    let resAudio = await axios.get(audioUrl, { responseType: 'arraybuffer' });
                    await amane.sendMessage(m.chat, { audio: Buffer.from(resAudio.data), mimetype: 'audio/mp4', contextInfo: audioContext }, { quoted: m });
                    audioSent = true;
                } catch (e) {
                }
            }
            ...
            if (!audioSent) {
                ...
                try {
                    ...
                    await amane.sendMessage(m.chat, { audio: fs.readFileSync(tmpAud), mimetype: 'audio/mp4', contextInfo: audioContext }, { quoted: m });
    ...
    case 'igimg': {
        if (!m.isGroup) return m.reply(global.mess.only.group);
        if (!text) return m.reply(`📌 Contoh: ${prefix + command} https://www.instagram.com/p/xxx/`);
            ...
            for (let i = 0; i < urls.length; i++) {
                try {
                    ...
                    if (mimeType.includes('video')) {
                        ...
                        if (!captionSent) {
                            await amane.sendMessage(m.chat, { text: captionText }, { quoted: m });
                            captionSent = true;
                        }
                        await amane.sendMessage(m.chat, {
                            audio: buffer,
    ...
    case 'igmusic': {
        if (!m.isGroup) return m.reply(global.mess.only.group);
        if (!text) return m.reply(`📌 Contoh: ${prefix + command} https://www.instagram.com/reel/xxx/`);
    ...
    case 'mp4': {
        ...
        if (!resolution) {
            try {
                ...
                await amane.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
            } catch (e) {
            ...
            try {
                ...
                await amane.sendMessage(m.chat, fileMsg, { quoted: m });
    ...
    case 'github':
    case 'gitclone': {
        if (!args[0]) return m.reply(`Mana linknya?\nExample :\n${prefix}${command} https://github.com/AhmadAkbarID/amanemd`);
...
// ====== MAKER FEATURE ======
    ...
    case 'brat': {
        ...
        try {
            ...
            await amane.sendMessage(m.chat, { sticker: webpBuffer }, { quoted: m });
    ...
    case 'iqc': {
        ...
        if (jam && !jam.includes(":")) return m.reply('❌ Format jam salah! Gunakan titik dua (:), contoh: 12:00');
        ...
        try {
            ...
            await amane.sendMessage(m.chat, { image: buffer }, { quoted: m });
    ...
    case 'quoted': {
        ...
        try {
            let ppUrl;
            try {
                ppUrl = await amane.profilePictureUrl(m.sender, 'image');
            } catch (e) {
            ...
            await amane.sendMessage(m.chat, { sticker: stickerBuffer }, { quoted: m });
    ...
    case 'igstory': {
        ...
        try {
            ...
            await amane.sendMessage(m.chat, { image: imageBuffer, caption: mess.success }, { quoted: m });
            if (cost > 0) {
                useLimit(m.sender, cost, Ahmad);
                fs.writeFileSync('./database/database.json', JSON.stringify(global.db, null, 2));
            }
    ...
    case 'bluearchivelogo': {
        ...
        if (!text.includes('|')) return m.reply('Format Salah! Teks kiri dan kanan harus dipisahkan dengan tanda |');

        const [l, r] = text.split('|');

        if (!l || !r) return m.reply('Format Salah! Teks kiri dan kanan tidak boleh kosong.');
    ...
    case 'pinterest': {
        ...
        try {
            ...
            let cards = await Promise.all(images.map(async (url, i) => {
                return {
                    header: proto.Message.InteractiveMessage.Header.create({
                        ...(await prepareWAMessageMedia({ image: { url: url } }, { upload: amane.waUploadToServer })),
                        title: '',
                    ...
                    message: {
                        ...
                        interactiveMessage: {
                            ...
                            header: {
                                ...
                                subtitle: "",
                                hasMediaAttachment: true,
                                ...(await prepareWAMessageMedia({ image: { url: video.thumbnail } }, { upload: amane.waUploadToServer }))
                            },
                            nativeFlowMessage: {
                                buttons: [
                                    {
...
// ====== CONVERTER FEATURE ======
    ...
    case 'tosticker': {
        ...
        if (!isImageMsg && !isVideoMsg) return m.reply(`📌 Reply gambar atau video dengan caption *${prefix + command}*`);
        ...
        try {
            ...
            exec(`ffmpeg -i ${tmpIn} ${ffmpegArgs} ${tmpOut}`, async (err) => {
                ...
                await amane.sendMessage(m.chat, { sticker: webp }, { quoted: m });
    ...
    case 'toimage': {
        ...
        if (!isStickerMsg) return m.reply(`📌 Reply stiker dengan caption *${prefix + command}*`);
        ...
        try {
            ...
            exec(`ffmpeg -i ${tmpIn} ${tmpOut}`, async (err) => {
                ...
                await amane.sendMessage(m.chat, { image: img, caption: mess.success }, { quoted: m });
                if (fs.existsSync(tmpOut)) fs.unlinkSync(tmpOut);
    ...
    case 'tovideo': {
        let isStickerMsg = m.quoted && m.quoted.mtype === 'stickerMessage';
        if (!isStickerMsg) return m.reply(`📌 Reply stiker (bergerak) dengan caption *${prefix + command}*`);
        ...
        try {
            ...
            exec(`ffmpeg -i ${tmpIn} -c:v libx264 -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ${tmpOut}`, async (err) => {
                ...
                await amane.sendMessage(m.chat, { video: vid, caption: mess.success }, { quoted: m });
                if (fs.existsSync(tmpOut)) fs.unlinkSync(tmpOut);
    ...
    case 'toaudio': {
        ...
        if (!isVideoMsg && !isAudioMsg) return m.reply(`📌 Reply video atau voice note dengan caption *${prefix + command}*`);
        ...
        try {
            ...
            exec(`ffmpeg -i ${tmpIn} -vn -b:a 128k ${tmpOut}`, async (err) => {
                ...
                await amane.sendMessage(m.chat, { audio: aud, mimetype: 'audio/mp4' }, { quoted: m });
                if (fs.existsSync(tmpOut)) fs.unlinkSync(tmpOut);
    ...
    case 'togif': {
        let isVideoMsg = type === 'videoMessage' || (m.quoted && m.quoted.mtype === 'videoMessage');
        if (!isVideoMsg) return m.reply(`📌 Reply video dengan caption *${prefix + command}*`);
    ...
    case 'todoc':
    case 'todocument': {
        if (!mime) return m.reply(`📌 Reply media apa saja (gambar, stiker, video, audio, dokumen) dengan caption *${prefix + command}*`);
    ...
    case 'toptt': {
        ...
        if (!isVideoMsg && !isAudioMsg) return m.reply(`📌 Reply video atau lagu/audio dengan caption *${prefix + command}*`);
    ...
    case 'wm': {
        let isStickerMsg = m.quoted && m.quoted.mtype === 'stickerMessage';
        if (!isStickerMsg) return m.reply(`⚠️ Reply stickernya dulu!`);
        ...
        try {
            ...
            await amane.sendMessage(m.chat, { sticker: newStickerBuffer }, { quoted: m });
...
// ====== ADDCASE ======
    ...
    case 'addcase': {
            if (!Ahmad) return m.reply(mess.only.owner)
            if (!q) return m.reply('Mana case nya');
            const namaFile = 'amane.js';
            const caseBaru = `${text}`;
...
`.trim();
            ...
            amane.sendMessage(m.chat, {
                ...
                contextInfo: {
                    externalAdReply: {
                        ...
                        sourceUrl: "https://store.amanehost.web.id",
                        mediaType: 1,
                        renderLargerThumbnail: true
                    }
                }
    ...
    case 'script': {
        try {
            ...
            try {
                let { data: pkgData } = await axios.get('https://raw.githubusercontent.com/AhmadAkbarID/amanemd/refs/heads/master/package.json');
                latestVersion = pkgData.version;
            } catch (e) {}

            let releases = [];
            try {
                let { data: relData } = await axios.get('https://api.github.com/repos/AhmadAkbarID/amanemd/releases');
                releases = relData; 
            } catch (e) {}

            let repoUrl = 'https://github.com/AhmadAkbarID/amanemd';
    ...
    case 'rating': {
        ...
        if (!isNaN(nilai)) {
            ...
            if (existing) {
                await react('❌');
                return m.reply('❌ Kamu sudah memberikan rating sebelumnya.\n`Terima kasih!`');
            }
            ...
            await amane.sendMessage(m.chat, quickMsg, { quoted: m });
            return await react('✅');
        }
    ...
    case 'cekrating': {
        ...
        if (!data || !data.length) {
            await react('✅');
            return m.reply(`⚠️ Belum ada pengguna yang memberikan rating.`);
        }
