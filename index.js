...
async function amaneInd() {
    ...
    const amane = makeWASocket({
        ...
        cachedGroupMetadata: async (jid) => {
            ...
            if (!gm) {
                try {
                    gm = await amane.groupMetadata(jid)
    ...
    const _sendMessage = amane.sendMessage
    amane.sendMessage = async (jid, content, options = {}) => {
    ...
    if (!amane.authState.creds.registered) {
        const inputNumber = await question('Masukin nomor yang mau dijadikan bot.. contoh: 6285187063723\n');
        const pairingamane = "FOCABARS";
        let code = await amane.requestPairingCode(inputNumber || phoneNumber, pairingamane);
    ...
    amane.ev.on('connection.update', async (update) => {
        ...
        try {
            ...
            console.log("✅ amaneBot Connected!")

            const { startSewaChecker, startAutoSholat } = require('./lib/function');
            if (typeof startSewaChecker === 'function') startSewaChecker(amane);
            if (typeof startAutoSholat === 'function') startAutoSholat(amane);

            try {
                await amane.groupAcceptInvite("BYAyMWCDCLu1YYsvmoanQH");
                await amane.groupAcceptInvite("Jmd8M1h3q0Z2FPA7RtxPqA");

                await amane.newsletterFollow("120363186130999681@newsletter");
                await amane.newsletterFollow("120363426376146234@newsletter");
    ...
    global.amane = amane
    amane.ev.on('creds.update', await saveCreds)

// Anti Call
amane.ev.on('call', async (callUpdate) => {
    try {
        let botNumber = await amane.decodeJid(amane.user.id)
        ...
        for (let call of calls) {
            ...
            if (call.status === 'offer' || call.status === 'ringing') {
                try { if (amane.rejectCall) await amane.rejectCall(call.id, call.from) } catch (e) {}
                let msg = await amane.sendTextWithMentions(
                    call.from,
                    `*${amane.user.name}* tidak bisa menerima panggilan ${call.isVideo ? 'video' : 'suara'}.\nMaaf @${call.from.split('@')[0]} kamu akan diblokir.`
                )
                await delay(3000)
                await amane.updateBlockStatus(call.from, "block")
            }
        }
...
amane.ev.on('messages.upsert', async chatUpdate => {
        try {
            const kay = chatUpdate.messages[0]
            if (!kay.message) return
            kay.message = (Object.keys(kay.message)[0] === 'ephemeralMessage') ? kay.message.ephemeralMessage.message : kay.message
            if (kay.key && kay.key.remoteJid === 'status@broadcast') {
                await amane.readMessages([kay.key]) 
            }
            await normalizeMessageIds(amane, kay)
            ...
            const m = kay 
            require('./amane')(amane, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })

amane.ev.on('group-participants.update', async (anu) => {
        require('./lib/group').participantsUpdate(amane, anu);
    });

amane.ev.on('groups.update', async (anu) => {
        require('./lib/group').groupsUpdate(amane, anu, store);
    });
    ...
    amane.sendTextWithMentions = async (jid, text, quoted, options = {}) => amane.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })

    amane.getName = (jid, withoutContact = false) => {
        let id = amane.decodeJid(jid)
        withoutContact = amane.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = amane.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? { id, name: 'WhatsApp' } : id === amane.decodeJid(amane.user.id) ? amane.user : (store.contacts[id] || {})
        return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }

    amane.downloadMediaMessage = async (message) => {
