...
async function listbut2(amane, m, teks, listnye) {
    let isVideo = false;
...
const amaneFitur = () => {
    try {
        const fileContent = fs.readFileSync('./amane.js', 'utf8');
...
function startSewaChecker(amane) {
    ...
    setInterval(async () => {
        ...
        if (shouldFetchGroups && hasPending) {
            try {
                activeGroups = await amane.groupFetchAllParticipating();
                lastFetchTime = now;
            } catch (e) {
                activeGroups = null;
            }
...
function startAutoSholat(amane) {
    setInterval(async () => {
