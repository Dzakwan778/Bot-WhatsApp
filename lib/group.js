...
module.exports.participantsUpdate = async (amane, anu) => {
    try {
        ...
        let metadata = await amane.groupMetadata(anu.id);
        ...
        for (let num of participants) {
            ...
            try {
                pp_user = await amane.profilePictureUrl(num, 'image');
            } catch {
                pp_user = 'https://i.ibb.co/1s8T3sY/48f7ce63c7aa.jpg';
            }

            let pushName = "amane User";
            ...
            if (anu.action === 'add') {
                ...
                await amane.sendMessage(anu.id, {
...
module.exports.groupsUpdate = async (amane, anu, store) => {
    try {
        for (let i of anu) {
            let from = i.id;
            if (!global.db.groups) global.db.groups = {};
