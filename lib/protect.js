...
async function antilinkDetector(amane, m, { budy, type, isAdmins, Ahmad, isBotAdmins, sender, senderNumber }) {
    ...
    if (detectedType && !isAdmins && !Ahmad && isBotAdmins) {
        ...
        try {
            await amane.sendMessage(m.chat, { delete: m.key });
        } catch (err) {}
        ...
        if (action === 'silent') {
            ...
            if (gc.warns[sender] >= gc.antilinkWarnLimit) {
                ...
                await amane.groupParticipantsUpdate(m.chat, [sender], 'remove');
                gc.warns[sender] = 0; 
            } else {
